...
def Hbeta(D=np.array([]), beta=1.0):
    ...
    P = np.exp(-D * beta)
    sumP = np.maximum(np.sqrt(sum(P)), 1e-12)
...
def regular_p(X=np.array([]), timesteps=1, tol=1e-5, perplexity=30.0):
    """
    ...
    for i in range(n):
        ...
        mask[[(n // timesteps) * t + i % (n // timesteps) for t in range(timesteps)]] = False
        Di = D[i, mask]
        (H, thisP) = Hbeta(Di, beta[i])
...
def ldtsne(Xs=np.array([]), Y_init=None, lX=np.array([]), lY=np.array([]),
    ...
    min_gain = 0.01
    n_timesteps = len(Xs)
    (n_points_per_timestep, n_dimensions) = Xs[0].shape

    dY = np.zeros((n_points_per_timestep, 2))
    dY_local = np.zeros((n_points_per_timestep, 2))
    dY_global = np.zeros((n_points_per_timestep, 2))
    iY = np.zeros((n_points_per_timestep, 2))
    gains = np.ones((n_points_per_timestep, 2))
    df_out = pd.DataFrame(index=index)
    if Y_init is None:
        Y = np.random.randn(n_points_per_timestep, 2)  # Random initialization
    else:
        Y = Y_init

    # Precompute P and lP for all timesteps
    _, L_betas = regular_p(lX, 1, 1e-5, perplexity)  # Compute betas for L
    lPs = []

    # Compute P-values and betas for Xs
    P, _ = regular_p(np.array(Xs).reshape((-1, n_dimensions)), n_timesteps, 1e-5, perplexity ** 2)
    P = P + np.transpose(P)
    P = P / (np.sum(P) / len(Xs))
    P = np.maximum(P, 1e-12)
    Ps = [P[t * n_points_per_timestep: (t + 1) * n_points_per_timestep,
          t * n_points_per_timestep: (t + 1) * n_points_per_timestep] for t in range(n_timesteps)]
    ...
    for iter in range(max_iter):        # Compute pairwise affinities
        ...
        num[range(n_points_per_timestep), range(n_points_per_timestep)] = 0.
        Q = num / np.sum(num)
        ...
        l_num = 1 / (1 + euclidian_distance(scaled_lY, Y))
        ...
        for i in range(n_points_per_timestep):
            dY_local[i, :] = (1 - l) * np.sum(np.tile( PQ[:, i] *   num[:, i], (2, 1)).T * (Y[i, :] - Y), 0)
            dY_global[i, :] = l  * np.sum(np.tile(lPQ[:, i] * l_num[:, i], (2, 1)).T * (Y[i, :] - scaled_lY), 0)
        ...
        Y = Y + iY
        Y = Y - np.tile(np.mean(Y, 0), (n_points_per_timestep, 1))
...
if __name__ == "__main__":
    ...
    Y = None
    Ys = []
    title = '{}-ldtsne-p{}--interactive-{}'.format(dataset_id, p, landmarks_info)
    # timestamp = str(int(time.time()))
    # print(title)

    ldtsne(Xs, Y, lX, lY, perplexity=p, max_iter=max_iter, title=title, plotter=pl, index=labels)
